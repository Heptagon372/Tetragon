# Tetragon v2 — 진단 · 설계 · 실행 계획 (rev.2 + 확장 08~16)
...
> **13~14 — 2026-08-23 추가. 신규 기능 기획(실측 키워드 루프)과 운영 리스크 관리 체계입니다.**
> **15~16 — 2026-08-25 추가. 그 둘을 실제 소스 코드에 붙이는 구현 설계입니다.** 기획이 아니라 티켓을 끊기 위한 문서입니다.
...
### 신규 기획 · 운영 체계 (2026-08-23 추가)

| 문서 | 내용 |
|---|---|
| [13-실측키워드루프.md](13-실측키워드루프.md) | **실측 키워드 루프 v1.1** — 자동 스크래핑 배제·업로드 퍼스트 구조 · 투입(등록 키워드)×회수(유입 키워드) 갭 분석 · 발굴/죽은/검증 키워드 · 대량등록 A/B 실험 · 138인일 로드맵 |
| [14-위탁판매-리스크관리.md](14-위탁판매-리스크관리.md) | **위탁판매 리스크관리 체계 v1.1** — 리스크 분류체계·평가 기준·레지스터 · 핵심 통제 상세 · KRI 모니터링 · 사고 대응 플레이북 · 증적 관리 · 실무 템플릿 |

### 구현 설계 (2026-08-25, 실제 소스 코드 기반)

13·14는 기획입니다. 아래 둘은 그 기획을 이 리포지토리의 실제 클래스·파일·스키마에 붙이는 설계이고, **선택지가 있는 곳마다 하나를 골라 근거를 남겼습니다.**

| 문서 | 내용 |
|---|---|
| [15-실측키워드루프-구현설계.md](15-실측키워드루프-구현설계.md) | **13의 전제가 코드에서 무너지는 지점부터** — 등록 시점 상품명·태그가 남지 않아 "투입×회수 조인"이 원천 불가라는 진단, `listing_snapshot` 신설 · 채널별 상품ID 회수 가부(ESM은 구조적 불가) · `keyword` 임포트 목적 확장 · **관측 부재를 유입 0으로 읽지 않는 4층 가드** · 사전 기반 한국어 분절 · 갭 분석(발굴/죽은/검증) · 주의 원장 연동 · 등록 실험 · 티켓 K-0~K-17 **58인일** |
| [16-위탁판매-리스크통제-구현설계.md](16-위탁판매-리스크통제-구현설계.md) | **14의 통제 64건을 [A]자동집행 / [B]코드보조 / [C]코드밖으로 전수 분류** — 등록 전 게이트(관찰 모드부터) · 금지·회색지대 필터 · KRI 10개 중 계산 가능한 것과 **불가능한 것을 불가능하다고 적기** · 증적 관리 · 사고 플레이북 상태 기계 · 개인정보 파기와 자격증명 봉인 · 집중도 · 티켓 RK-01~RK-21 **70.5인일** |

두 문서는 실사 7건 → 설계 → 적대적 검증(코드 인용 사실검증 + blocker 지적) → 개정 → 재검증을 거쳤습니다. 인용한 파일 경로·줄 번호·시그니처는 검증 시점의 실측값입니다.
